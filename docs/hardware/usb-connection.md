# USB Connection

All VNM devices communicate with the computer through USB connections (USB 2.0 or USB 3.0 ports on the PC).

Each device should be connected directly to the PC using its dedicated USB cable.

## Typical Connection Layout

| Device | Connection |
|---|---|
| Wheelbase | USB → PC |
| Pedals | USB → PC |
| Shifter | USB → PC |
| Handbrake | USB → PC |
| Button Box | USB → PC |
| Motion Controller | USB → PC |

## Recommendations

!!! tip "Best Practice"
    For best performance, connect devices **directly to the motherboard USB ports** on the computer.

- Avoid using low-quality USB hubs, as they may cause unstable device communication.
- If a USB hub must be used, use a **powered USB hub** with sufficient power capacity.
